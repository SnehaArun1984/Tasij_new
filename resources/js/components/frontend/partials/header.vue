<template>
	<header class="sg-header home-4" :class="{ 'sidebar-active': smCategory, 'mobile-menu-active': show_sm_home }">
				...
				<div class="topbar-content">
						...
						<ul class="global-list d-flex align-items-center">
							<li v-if="settings.language_switcher != 0">
								<div class="dropdown">
									<button class="dropdown-toggle" type="button" @click.stop="languageDropdown"
										:class="{ show: language_dropdown }" data-bs-toggle="dropdown"
										aria-expanded="false">{{ activeLanguage.name }} </button>
									<ul @click.stop class="dropdown-menu" :class="{ show: language_dropdown }"
										aria-labelledby="">
										<li v-for="(language, index) in languages" :key="index">
											<a class="dropdown-item" @click="changeLanguage(language.locale)"
												href="javascript:void(0)"><img :src="language.flag_image" alt="flag"
													class="img-fluid" />{{ language.name }}</a>
										</li>
									</ul>
								</div>
							</li>
							<li v-if="settings.currency_switcher != 0">
								<div class="dropdown">
									<button class="dropdown-toggle" type="button" id="" @click.stop="currencyDropdown"
										:class="{ show: currency_dropdown }" data-bs-toggle="dropdown"
										aria-expanded="false">{{ activeCurrency.name }} </button>
									<ul @click.stop class="dropdown-menu" :class="{ show: currency_dropdown }"
										aria-labelledby="">
										<li v-for="(currency, index) in currencies" :key="index"><a class="dropdown-item"
												@click="changeCurrency(currency)" href="javascript:void(0)">{{ currency.name
												}}</a></li>
									</ul>
								</div>
							</li>
							<li v-if="settings.topbar_play_store_link == 1">
								<a :href="settings.play_store_link" target="_blank"><span
										class="mdi mdi-name mdi-google-play"></span> <small>{{ lang.play_store
										}}</small></a>
							</li>
							<li v-if="settings.topbar_app_store_link == 1">
								<a :href="settings.apple_store_link" target="_blank"><span
										class="mdi mdi-name mdi-apple"></span> <small>{{ lang.app_store }}</small></a>
							</li>
					...
					<div class="right-content new-list">
						<ul class="global-list">
							<li>
								<div class="location" style="background-color: #f5f5f5; text-align: center;">
									<div class="dropdown"><button class="dropbtn"><span
												class="mdi mdi-name mdi-map-marker-outline"></span> Deliver to Sneha Arun,
											Trivandrum 695583</button>
										<div class="dropdown-content"><a href="#"><span
													class="mdi mdi-name mdi-map-marker-outline"></span> Manu George, Kollam
												695593</a> <a href="#"><span
													class="mdi mdi-name mdi-map-marker-outline"></span> Arathi James,
												Kazhakootam 695588</a> <a href="#"><span
													class="mdi mdi-name mdi-map-marker-outline"></span> Parvathy s,
												Trivandrum 695553</a></div>
									</div>
								</div>
							</li>
							<li>
								<router-link :to="{ name: 'track.order' }"><span
										class="mdi mdi-name mdi-map-marker-outline"></span>
									{{ lang.track_order }}
								</router-link>
							</li>
							<!-- <li>
		...
		<div :class="addons.includes('ishopet') ? toggleNavClass() : ''" id="middle_nav" class="header-middle">
			<div class="container">
				<div class="botom-content">
					<div class="sg-logo">
						<div class="sg-categorie-menu categorie-lg align-self-lg-center"><!----> <!----></div>
						<a href="#" aria-current="page" class="router-link-exact-active router-link-active">
							<img src="https://tasij.lifeashorecrm.com/public/images/default/dark-logo.png" alt="Logo"
								class="img-fluid">
						</a>
					</div>
					...
					<div class="sg-search">
						<div class="search-form">
							<div class="btn-group w-100">
								<input v-model="searchKey" type="text" class="form-control" :placeholder="lang.looking_for"
								...
								<ul @click.stop class="dropdown-menu dropdown-menu-right show w-100 search_dropdown"
									...
									<li v-for="(product, index) in search_products" :key="index" class="dropdown-item"
										@click.prevent="routerNavigator('product.details', product.slug)">
										<div class="search-show-result">
											<div class="search-suggestion">
												<img class="img-fluid" :src="product.image_40x40"
						...
						<ul class="global-list user-shop-option">
							<li>
								<router-link :to="{ name: 'wishlist' }" v-if="authUser && authUser.user_type == 'customer'">
									<div class="icon"><img alt="Compare Icon" class="img-fluid"
											:src="getUrl('public/images/others/wishlist.svg')" />
								...
								<a class="d-block" :href="getUrl('wishlist')" v-else>
									<div class="icon"><img alt="Compare Icon" class="img-fluid"
											:src="getUrl('public/images/others/wishlist.svg')" />
										<!---->
									</div>
							...
							<li class="sg-dropdown cart">
								<router-link :to="{ name: 'cart' }" class="">
									<div class="icon"><img alt="bag Icon" class="img-fluid"
										...
										<span v-if="carts && carts.length > 0" class="badge">{{ carts.filter(cart =>
											cart.is_buy_now == false).length }}</span>
									</div>
								</router-link>
									...
									<ul class="global-list">
										<li v-for="(cart, index) in carts" :key="index">
											<div class="sg-product" v-if="!cart.is_buy_now">
												<span @click="deleteCart(cart.id)"
													class="remove-icon mdi mdi-name mdi-close"></span>
												...
												<div class="product-info text-ellipse">
													...
													<span class="price">{{ priceFormat(cart.price - cart.discount) }} x {{
														cart.quantity }}</span>
												</div>
											</div>
										</li>
										<li>
											<div class="text-center buttons">
												...
												<a :href="getUrl('checkout')" class="btn btn-primary"
													@click="checkoutPage($event)">{{ lang.check_out }}</a>
											</div>
										</li>
									</ul>
								</div>
							...
							<li class="compare-icon">
								<router-link :to="{ name: 'compare.list' }">
									<div class="icon"><img :src="getUrl('public/images/others/compare.svg')" alt="List" />
									</div>
							...
							<li class="d-flex user-log-info">
								...
								<div class="text-left text" v-if="authUser">
									...
									<a class="d-block" :href="getUrl('admin/dashboard')"
										v-if="authUser.user_type == 'admin' || authUser.user_type == 'staff'">{{
											lang.dashboard }}</a>
									...
									<ul class="navbar-nav">
										<li class="nav-item" v-for="(menu, i) in headerMenu" :key="'menu' + i"
											...
											<a v-if="urlCheck(menu.url)" :href="menu.url" @click="subMenuToggle"
												@mouseover="subMenuToggle">{{ menu.label }}<span
											...
											<ul class="sg-dropdown-menu" :class="{ 'd-block': subMenu, 'd-none': !subMenu }"
												...
												<li v-for="(sub_menu, j) in menu" :key="'sub_menu' + j" v-if="sub_menu.url">
													<a v-if="urlCheck(sub_menu.url)" :href="sub_menu.url">{{ sub_menu.label
													}}</a>
													<router-link v-else :to="sub_menu.url">{{ sub_menu.label
													}}</router-link>
												</li>
											</ul>
										</li>
		...
		<div class="sg-categorie-menu home-menu-sm">
			<div class="sg-toggle" @click="$store.commit('setSmCategory', false)"><span
					class="mdi mdi-name mdi-close"></span></div>
			<div class="categorie-menu-content">
				<div class="sg-logo">
					<router-link :to="{ name: 'home' }">
						<svg v-if="settings.demo_mode" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
							...
							<path class="st0" d="M103.23,45.6c-14.37,48.11-40.59,89.7-70.83,115.93H0c11.36-9.86,21.21-19.54,29.9-29.23L11.86,45.6h30.4
							...
							<path class="st1" d="M390.2,17.21C390.2,5.85,398.22,0,409.24,0c10.02,0,16.7,5.18,16.7,14.53c0,11.69-8.02,17.04-19.04,17.04
				...
				<ul @click.stop class="global-list">
					<li class="nav-item" v-for="(menu, i) in headerMenu" :key="'menu' + i"
						:class="{ 'sg-dropdown': Object.keys(menu).length > 2 }">
						<a v-if="urlCheck(menu.url)" :href="menu.url">{{ menu.label }}<span
								v-if="Object.keys(menu).length > 2" class="icon mdi mdi-name mdi-chevron-down"></span></a>
						<router-link v-else :to="menu.url" :class="{ active: menu.url == $route.fullPath }"> {{ menu.label
						}}<span v-if="Object.keys(menu).length > 2"
								class="icon mdi mdi-name mdi-chevron-down"></span></router-link>
						<ul class="sg-dropdown-menu"
							:class="{ 'd-block': home_child_id == menu.id, 'd-none': home_child_id != menu.id }"
							v-if="Object.keys(menu).length > 2">
							<li v-for="(sub_menu, j) in menu" :key="'sub_menu' + j" v-if="sub_menu.url">
						...
						<span v-if="Object.keys(menu).length > 2"
							@click="home_child_id == menu.id ? (home_child_id = 0) : (home_child_id = menu.id)"
							class="icon mdi mdi-name mdi-chevron-down"></span>
					</li>
				</ul>
			</div>
		...
		<div class="sg-categorie-menu categorie-sm">
			<div @click="$store.commit('setSmCategory', false)" class="sg-toggle"><span
					class="mdi mdi-name mdi-close"></span></div>
			<div class="categorie-menu-content">
				<div class="sg-logo">
					<router-link :to="{ name: 'home' }">
				...
				<ul @click.stop class="global-list">
					<li class="active" @click="$store.commit('setSmCategory', false)">
						<router-link :to="{ name: 'campaigns' }"><span
								class="mdi mdi-view-dashboard-outline mdi-18px"></span>{{ lang.all_offers }} </router-link>
					</li>
						...
						<ul class="sg-dropdown-menu"
							...
							<li class="active" v-for="(child_category, index) in category.child_categories" :key="index"
								...
								<router-link :to="{ name: 'product.by.category', params: { slug: child_category.slug } }">
									<p class="mb-0" @click="$store.commit('setSmCategory', false)">{{ child_category.title
									}}</p>
								</router-link>
								<ul class="sg-dropdown-menu"
					...
					<ul class="global-list">
						...
						<li>
							<div v-if="authUser">
								...
								<a class="d-block" :href="getUrl('seller/dashboard')"
									v-else-if="authUser.user_type == 'seller'"><img
						...
						<div class="row">
							...
							<div class="col-lg-8">
								<div class="row">
									<div class="col-md-8">
										<div class="products-details-info">
											...
											<div class="stock-delivery">
												<shimmer class="de-margin" v-for="(list, i) in 2" :key="i" :height="100">
												</shimmer>
											</div>
											<div class="row">
									...
									<div class="col-md-4">
										<shimmer class="mb-3" v-for="(list, i) in 2" :key="i"
											:height="list == 1 ? 200 : 300"></shimmer>
									</div>
								</div>
							</div>
						</div>
